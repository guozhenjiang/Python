...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.windowTitleChanged.connect(self.onWindowTitleChane)        # 窗口标题发生变化时，槽函数被调用，新的标题作为参数

        # SIGNAL: The connected function will be called whenever the window
        # title is changed. The new title is discarded in the lambda and the
        # function is called without parameters.
        self.windowTitleChanged.connect(lambda x: self.my_custom_fn())  # 窗口标题发生变化时，槽函数被调用，传入的参数均被丢弃

        # SIGNAL: The connected function will be called whenever the window
        # title is changed. The new title is passed to the function
        # and replaces the default parameter
        self.windowTitleChanged.connect(lambda x: self.my_custom_fn(x)) # 窗口标题发生变化时，槽函数被调用，新的标题作为参数传入

        # SIGNAL: The connected function will be called whenever the window
        # title is changed. The new title is passed to the function
        # and replaces the default parameter. Extra data is passed from
        # within the lambda.
        self.windowTitleChanged.connect(lambda x: self.my_custom_fn(x, 25)) # 窗口标题发生变化时，槽函数被调用，新的标题作为参数传入，还可传递更多参数
