...
class Pkg_nTnA():
    ...
    def update(self, raw_bytes):
        if (0xFF == raw_bytes[0]) and (0x02 == raw_bytes[1]):
            self.info = {}
            self.info['stamp'] = time_stamp_ms()
            self.info['tags'] = {}
            
            tag_id = 0
            self.info['tags'][tag_id] = {}
            
            self.info['tags'][tag_id]['x'] = int(np.int16((raw_bytes[ 9]  << 8) | raw_bytes[10]))
            self.info['tags'][tag_id]['y'] = int(np.int16((raw_bytes[11]  << 8) | raw_bytes[12]))
            
            self.info['tags'][tag_id]['d'] = {}
            d_id = 0
            self.info['tags'][tag_id]['d'][d_id] = (raw_bytes[13]  << 8) | raw_bytes[14]
            d_id = 1
            self.info['tags'][tag_id]['d'][d_id] = (raw_bytes[15]  << 8) | raw_bytes[16]
            d_id = 2
            self.info['tags'][tag_id]['d'][d_id] = (raw_bytes[17]  << 8) | raw_bytes[18]
            
            self.info['tags'][tag_id]['r'] = {}
            r_id = 0
            self.info['tags'][tag_id]['r'][r_id] = int(np.int16((raw_bytes[19]  << 8) | raw_bytes[20]))
            r_id = 1
            self.info['tags'][tag_id]['r'][r_id] = int(np.int16((raw_bytes[21]  << 8) | raw_bytes[11]))
            r_id = 2
            self.info['tags'][tag_id]['r'][r_id] = int(np.int16((raw_bytes[23]  << 8) | raw_bytes[24]))
...
class IndoorLocation(QObject):
    def __init__(self):
        ...
        self.new_pkg = Pkg_nTnA()       # 串口数据包解析缓存
