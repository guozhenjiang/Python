...
csv_file = open('./vhub/20-03-03 03-15-01_111.csv', 'r')
...
layout.addWidget(cb_keepout, row=0, col=0)
...
def update_ui_by_uart():
    global win_data, uart_byte_in_pkg_index, cb_stop
...
def update_ui_by_csv():
    global win_data, index_cvs_row, cb_stop

    if(not cb_stop.isChecked()):
        win_data = win_data[csv_row_ppg_num:] # 通过切片给新数据腾出位置
